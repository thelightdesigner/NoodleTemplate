...
namespace NoodleTemplate
{
    public class TreeDictionary : Dictionary<string, object>, IDictionary<string, object>, ICloneable
    {
        ...
        public TreeDictionary merge(IDictionary<string, object> Dictionary2, MergeType mergeType = MergeType.Dictionaries | MergeType.Objects | MergeType.Arrays, MergeBindingFlags mergeBindingFlags = MergeBindingFlags.HasValue) => Merge(this, Dictionary2, mergeType, mergeBindingFlags);
    ...
    /*
    public class TreeListJsonConverter : JsonConverter<List<object>>
    {
        public override List<object> Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            if (reader.TokenType != JsonTokenType.StartArray) throw new JsonException($"JsonTokenType was of type {reader.TokenType}, only objects are supported");

            var list = new List<object>();
            while (reader.Read())
            {
                if (reader.TokenType == JsonTokenType.EndArray) return list;
                else list.Add(TreeDictionaryJsonConverter.ExtractValue(ref reader, options));
            }
    ...
    */
    ...
    {
        public static JsonSerializerOptions TreeDictionaryOptions
        {
            get
            {
                JsonSerializerOptions jso = new JsonSerializerOptions() { WriteIndented = true};
                jso.Converters.Add(new TreeDictionaryJsonConverter());
                return jso;
            }
        }
        ...
        public object ExtractValue(ref Utf8JsonReader reader, JsonSerializerOptions options)
        {
